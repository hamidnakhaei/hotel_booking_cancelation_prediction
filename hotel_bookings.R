...
#########in table ha va plot vase chiye?
#table(h$adults)
#table(h$children)
#table(h$babies)
#ggplot(data = h, aes(x=num,fill=is_canceled_f )) + geom_bar()
...
city_Matrix_lg <- table(h2_test$is_canceled_f, pred_test_log > 0.35)
...
set.seed(1725)
resort_rfmod <- randomForest(is_canceled_f ~ . , data=h1_train, method="class", nodsize=10, ntree=500, mtry=def_mtry)

resort_pred_train_rf <- predict(resort_rfmod, type="class")
#draw confusion matrix-train
resort_train_Matrix_rf <- table(h1_train$is_canceled_f, resort_pred_train_rf)
...
city_pred_train_rf <- predict(city_rfmod, type="class")
#draw confusion matrix-train
city_train_Matrix_rf <- table(h2_train$is_canceled_f, city_pred_train_rf)
